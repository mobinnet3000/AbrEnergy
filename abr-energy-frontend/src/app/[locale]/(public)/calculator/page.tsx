...
export default function CalculatorPage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      toast.success(t('calculator.title'));
    ...
    <div className="bg-black min-h-screen">
      ...
      <section className="relative pt-28 pb-16 md:pt-36 md:pb-24 overflow-hidden">
        ...
        <div className="container-page relative z-10">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-white/35 text-base max-w-lg">{t('calculator.subtitle')}</motion.p>
        </div>
      </section>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    <FormField control={form.control} name="daily_consumption" render={({ field }) => (
                      <FormItem><FormLabel className="text-xs text-white/50">{t('calculator.daily_consumption')}</FormLabel><FormControl><Input type="number" step="0.1" className="bg-white/[0.03] border-white/[0.08] text-white" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="city" render={({ field }) => (
                      <FormItem><FormLabel className="text-xs text-white/50">{t('calculator.city')}</FormLabel><FormControl><Input placeholder="Tehran" className="bg-white/[0.03] border-white/[0.08] text-white" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="irradiation" render={({ field }) => (
                      <FormItem><FormLabel className="text-xs text-white/50">{t('calculator.irradiation')}</FormLabel><FormControl><Input type="number" step="0.1" placeholder="5.0" className="bg-white/[0.03] border-white/[0.08] text-white" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="battery_type" render={({ field }) => (
                      <FormItem><FormLabel className="text-xs text-white/50">{t('calculator.battery_type')}</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className="bg-white/[0.03] border-white/[0.08] text-white"><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="lithium">{t('calculator.lithium')}</SelectItem><SelectItem value="lead_acid">{t('calculator.lead_acid')}</SelectItem><SelectItem value="tubular">{t('calculator.tubular')}</SelectItem></SelectContent></Select><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="system_type" render={({ field }) => (
                      <FormItem><FormLabel className="text-xs text-white/50">{t('calculator.system_type')}</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className="bg-white/[0.03] border-white/[0.08] text-white"><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="off_grid">{t('calculator.off_grid')}</SelectItem><SelectItem value="on_grid">{t('calculator.on_grid')}</SelectItem><SelectItem value="hybrid">{t('calculator.hybrid')}</SelectItem></SelectContent></Select><FormMessage /></FormItem>
                    )} />
            ...
            <div className="space-y-5">
              {result ? (
                <ScrollReveal variant="scale">
                  <div className="p-6 md:p-8 rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-lg">
                    <h2 className="font-heading font-semibold text-lg text-white mb-6 flex items-center gap-2"><Sparkles className="h-5 text-emerald-400" /> {t('calculator.results')}</h2>
                    <div className="grid grid-cols-2 gap-3">
                      <ResultCard icon={Sun} value={String(result.panel_count)} label={t('calculator.panels')} />
                      <ResultCard icon={Zap} value={String(result.panel_capacity)} label={t('calculator.panel_capacity')} />
                      <ResultCard icon={BarChart3} value={result.battery_capacity.toFixed(1)} label={t('calculator.battery_capacity')} />
                      <ResultCard icon={Zap} value={String(result.inverter_power)} label={t('calculator.inverter_power')} />
                    </div>
                    <div className="grid grid-cols-2 gap-3 mt-3">
                      <ResultCard icon={DollarSign} value={formatCurrency(result.estimated_cost)} label={t('calculator.estimated_cost')} />
                      <ResultCard icon={Clock} value={String(result.roi_years)} label={t('calculator.roi')} />
                    </div>
